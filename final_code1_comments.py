...
def get_supported_currencies():
    ...
    try:
        ...
        if data.get("success"):
            ...
            st.error("⚠️ API returned an error: " + data.get("error", {}).get("info", "Unknown error"))
            return fallback_currencies

    except requests.exceptions.RequestException as e:
        st.error(f"⚠️ Error fetching currency data: {e}")
        return fallback_currencies

# Use the function to get currencies
currencies = get_supported_currencies()

# Load data from files — do this BEFORE any session logic - a very important step
def load_data():
...
def create_account():
    ...
    st.markdown("### Welcome to Budget Friend! 💸")
    st.markdown("**Start your financial journey today! You're one step closer to mastering your budget. 💪**")
...
def login():
    ...
    st.markdown("### Welcome to Budget Friend! 💸")
    st.markdown("**You're doing great! Let's keep those finances on track. 💪**")
...
def choose_time():
    ...
    if current_year:
        if current_year.isdigit() and len(current_year) == 4:
            st.success(f"✔ Valid year entered: {current_year}")
        else:
            st.error("✘ Please enter a valid year (e.g. 2025).")
            return None
    else:
        return None
    ...
    st.success(f"✅ New month and year defined for {key.replace('_', ' ')}.")
    return key

#DEFINE BUDGET FUNCTION
def define_budget():
    st.subheader("📅 Define your monthly Budget")
    key = choose_time()
    if not key:
        return
    ...
    if budget_type == "Expected Income":
        manage_budget_for_category(key)
    elif budget_type == "Expected Expenses":
        manage_expense_budget(key)


def manage_budget_for_category(key):
    """Add/Edit/Delete expected income budget for the selected period."""
    ...
    if action == "Add":
        ...
        if st.button("➕ Add Income Budget"):
            current_value = user_data[key].get(field, 0.0)
            if isinstance(current_value, dict):
                current_value = current_value.get("amount", 0.0)
            user_data[key][field] = current_value + amount
        ...
        if field in user_data[key]:
            ...
            amount = current_value.get("amount", 0.0) if isinstance(current_value, dict) else current_value
            new_amount = st.number_input("New budget amount:", value=amount, min_value=0.0, format="%.2f")
    ...
    display_budget_summary(key, income_sources, "expected", currency)


def manage_expense_budget(key):
    """Add/Edit/Delete expected expense budget for the selected period."""
    if st.session_state.username not in st.session_state.data:
        st.session_state.data[st.session_state.username] = {}
    ...
    if action == "Add":
        ...
        if field in user_data[key]:
            if st.button("🗑 Delete Expense Budget"):
                del user_data[key][field]
                st.success("✔ Expense budget deleted.")
                data_changed = True
        else:
            st.warning("⚠ No budget found to delete.")
    ...
    display_budget_summary(key, expense_categories, "expected", currency)


def display_budget_summary(key, category_list, prefix, currency):
    """Display summary of budgets for the selected categories."""
    st.markdown("---")
    st.subheader(f"📊 {'Income' if prefix == 'expected' and category_list == income_sources else 'Expense'} Budget Summary")

    user_data = st.session_state.data.get(st.session_state.username, {})
    data = user_data.get(key, {})

    if not data:
        st.info("No budget entries for this period yet.")
        return

    total = 0.0
    for category in category_list:
        field_key = f"{prefix}_{category.lower()}"
        value = data.get(field_key)
        if value:
            amount = value.get("amount", value) if isinstance(value, dict) else value
            st.markdown(f"• **{category}**: {amount:.2f} {currency}")
            total += amount

    st.subheader(f"**Total Budgeted {'Income' if prefix == 'expected' and category_list == income_sources else 'Expenses'}:** {total:.2f} {currency}")


def add_income():
    st.subheader("💰 Register Income")
    st.markdown(
        "Add, update or delete your income entries for the selected month and year. "
    ...
    if action == "Add":
        ...
        if st.button("➕ Add Income", type="primary"):
            current_amount = user_data[key].get(selected_source, 0.0)
            if isinstance(current_amount, dict):
                current_amount = current_amount.get("amount", 0.0)
            user_data[key][selected_source] = current_amount + amount
            st.success(f"✔ Added {amount:.2f} {currency} for {selected_source}.")
            save_data()

    elif action == "Edit":
        if selected_source in user_data[key]:
            current_amount = user_data[key][selected_source]
            amount = current_amount.get("amount", current_amount) if isinstance(current_amount, dict) else current_amount
            new_amount = st.number_input(f"New amount for {selected_source}", value=amount, min_value=0.0, format="%.2f")
            if st.button("✏️ Update Income"):
                user_data[key][selected_source] = new_amount
                st.success("✔ Income updated.")
                save_data()
        else:
            st.warning(f"⚠ No income entry found for {selected_source}.")
    ...
    display_budget_summary(key, income_sources, "", currency)


def add_expense():
    st.subheader("💸 Register Expense")
    st.markdown(
        "Add, update or delete your expense entries for the selected month and year. "
        "All changes are saved automatically."
    )
    ...
    col1, col2 = st.columns(2)
    with col1:
        selected_category = st.selectbox("💸 Expense category", expense_categories)
    with col2:
        action = st.radio("⚙️ Action", ["Add", "Edit", "Delete"], horizontal=True)
    ...
    if action == "Add":
        ...
        if st.button("➕ Add Expense", type="primary"):
            current_amount = user_data[key].get(selected_category, 0.0)
            if isinstance(current_amount, dict):
                current_amount = current_amount.get("amount", 0.0)
        ...
        if selected_category in user_data[key]:
            current_amount = user_data[key][selected_category]
            amount = current_amount.get("amount", current_amount) if isinstance(current_amount, dict) else current_amount
            new_amount = st.number_input(f"New amount for {selected_category}", value=amount, min_value=0.0, format="%.2f")
            if st.button("✏️ Update Expense"):
                user_data[key][selected_category] = new_amount
                st.success("✔ Expense updated.")
                save_data()
        else:
            st.warning(f"⚠ No expense entry found for {selected_category}.")

    elif action == "Delete":
        if selected_category in user_data[key]:
            if st.button("🗑 Delete Expense"):
                del user_data[key][selected_category]
                st.success("✔ Expense deleted.")
                save_data()
        else:
            st.warning(f"⚠ No expense entry found for {selected_category}.")

    display_budget_summary(key, expense_categories, "", currency)
...
def view_single_month_summary():
    ...
    if st.session_state.username not in st.session_state.data or not st.session_state.data[st.session_state.username]:
        st.warning("⚠️ No data available. Please add some transactions first.")
        return
    #acess the data of teh user
    user_data = st.session_state.data[st.session_state.username]
    #avaiçabel months of the user - certifuing is a two part (to avoid appearing currency)
    available_keys = [
    ...
    selected_month = st.selectbox("📆 Select a month to view transactions:", [""] + month_options)
    ...
    select_summary = st.selectbox("📊 Select a summary:", ["Income Summary","Expense Summary", "Profit Summary", "Actual vs Expectations Graphs"])
...
def compare_income(month, user_data):
    st.subheader(f"📊 Income Summary for {month.replace('_', ' ').title()}:\n")
    ...
    for source in income_sources:
        ...
        if expected_amount > 0:
            if actual_amount >= expected_amount:
                status = f"✅ Over by {diff:.2f} {currency}"
            else:
                status = f"❌ Under by {-diff:.2f} {currency}"
        else:
            status = "⚠️ No expected income"
...
def compare_expenses(month, user_data):
    st.subheader(f"📊 Expense Summary for {month.replace('_', ' ').title()}:\n")
    ...
    for category in expense_categories:
        ...
        if expected_amount > 0:
            ...
            if percentage >= 100:
                ...
                status = f"⚠️ {percentage:.1f}% used"
            else:
                status = "✅ Good job!"
        else:
            status = "⚠️ No expected budget"
...
def compare_total_income_expenses(month, user_data):
    ...
    #final summary
    st.write(f"➕ **Total Income:** {income_total:.2f} {currency}")
    st.write(f"➖ **Total Expenses:** {expense_total:.2f} {currency}")
    st.write(f"📉 **Net:** {net:.2f} {currency} ({'🟢 Surplus' if net >= 0 else '🔴 Deficit'})")
...
def plot_income_vs_budget(month, user_data):
    st.subheader(f"💼 Real Income vs Budget Income - {month.replace('_', ' ').title()}")
    ...
    if not display_sources:
        st.info("ℹ️ There is no income data to display for this month.")
        return
...
def plot_expenses_vs_budget(month, user_data):
    ...
    if not display_categories:
        st.info("ℹ️ There are no expenses to display for this month.")
        return
    ...
    st.subheader("💼 Real Expenses vs Budget Expenses")
    st.pyplot(fig)
...
def get_months_to_compare():
    ...
    if not user_data:
        st.warning("⚠️ No data available to compare. Please add transactions.")
        return None, None
    ...
    if not available_keys or len(available_keys) < 2:
        st.warning("⚠️ Not enough data to compare. Please add data for at least two months.")
        return None, None
    ...
    col1, col2 = st.columns(2)
    with col1:
        month1_display = st.selectbox("📅 Select the first month:", month_options, key="compare_month1")
    # Filter out the selected first month from the second list
    filtered_months = [m for m in month_options if m != month1_display]
    with col2:
        month2_display = st.selectbox("📅 Select the second month:", filtered_months, key="compare_month2")
...
def compare_multiple_months():
    ...
    if comparison_type == "Overall Comparison":
        ...
        data = [
            {"Month": month1.replace('_', ' ').title(),
             ...
             "Status": "🟢 Surplus" if profit1 >= 0 else "🔴 Deficit"},
    ...
    #income comparison of 2 months
        ...
        source = st.selectbox("💰 Select income source:", income_sources, key="income_source_selector")
    ...
    #expenses comparison = for 2 months
        ...
        category = st.selectbox("💸 Select expense category:", expense_categories, key="expense_category_selector")
        ...
        if all(v is None or v == 0 for v in [actual1, expected1, actual2, expected2]):
            st.warning("⚠️ No expense data available for the selected category in either month.")
        else:
            st.table(pd.DataFrame(rows))
...
def view_transactions():
    ...
    if not st.session_state.get("username"):
        st.warning("⚠️ You must be logged in to view transactions.")
        return

    user_data = st.session_state.data.get(st.session_state.username, {})
    if not user_data:
        st.warning("⚠️ No transaction data available for this user.")
        return
    ...
    selected_months = st.multiselect("📆 Select months to view:", display_names)
    ...
    show_chart = st.checkbox("📊 Show Income and Expense Overview (Graph)", value=True)

    if show_chart and chart_data["Income"]:
        st.subheader("📊 Income and Expense Overview")
...
def dashboard():
    # Sidebar welcome
    st.sidebar.markdown("#### 📝 Welcome to Budget Friend")
